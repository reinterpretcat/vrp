...
fn can_validate_permutations() {
    ...
    assert!(permutator.validate(&[0, 2, 1, 3, 4]));
    assert!(permutator.validate(&[0, 2, 1, 4, 3]));

    assert!(!permutator.validate(&[]));
    assert!(!permutator.validate(&[0]));
    assert!(!permutator.validate(&[0, 3, 2, 1, 4]));
    assert!(!permutator.validate(&[0, 1, 3, 2, 4]));
    assert!(!permutator.validate(&[0, 1, 1, 4, 3]));
