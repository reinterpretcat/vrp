...
use vrp_core::construction::features::LocalObjective;
...
fn can_handle_reload_jobs_with_merge() {
    ...
    let feature = create_simple_reload_multi_trip(
        Box::new(|multi_trip| create_capacity_limit_with_multi_trip(VIOLATION_CODE, multi_trip)),
...
fn can_remove_trivial_reloads_when_used_from_capacity_constraint_impl(
    ...
    let feature = create_simple_reload_multi_trip::<MultiDimLoad>(
        Box::new(|multi_trip| create_capacity_limit_with_multi_trip(VIOLATION_CODE, multi_trip)),
        Box::new(move |capacity| *capacity * threshold),
    )
    .unwrap();
    let registry = LocalObjective::new(&[vec![feature]]).unwrap();

    registry.accept_route_state(solution_ctx.routes.get_mut(0).unwrap());

    registry.accept_solution_state(&mut solution_ctx);
