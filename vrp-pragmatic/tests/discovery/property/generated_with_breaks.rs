...
fn get_optional_breaks() -> impl Strategy<Value = Option<Vec<VehicleBreak>>> {
    let places_proto = get_optional_break_places(get_optional_break_locations(), generate_durations(10..100));
    let break_proto = generate_break(prop::collection::vec(places_proto, 1..2), get_optional_break_times(), Just(None));

    prop::collection::vec(break_proto, 1..2).prop_map(|break_| Some(break_))
}

fn get_optional_break_locations() -> impl Strategy<Value = Option<Location>> {
...
fn get_optional_break_times() -> impl Strategy<Value = VehicleOptionalBreakTime> {
    prop_oneof![get_break_offset_time(), get_optional_break_time_windows()]
}
...
prop_compose! {
    fn get_problem_with_optional_breaks()
    (
...
proptest! {
    ...
    #[test]
    #[ignore]
    fn can_solve_problem_with_optional_breaks(problem in get_problem_with_optional_breaks()) {
