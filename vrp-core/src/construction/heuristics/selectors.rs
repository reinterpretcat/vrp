...
impl BlinkResultSelector {
    ...
    pub fn new(ratio: f64, random: Random) -> Self {
        Self { random, ratio }
    }
...
impl ResultSelectorProvider {
    ...
    pub fn new_default(random: Random) -> Self {
        Self {
            inners: vec![
...
impl LegSelection {
    ...
    fn get_sample_data(&self, route_ctx: &RouteContext, job: &Job, skip: usize) -> Option<(usize, Random)> {
        match self {
