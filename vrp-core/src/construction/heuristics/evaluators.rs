...
pub fn evaluate_job_insertion_in_route(
    ...
    let route_costs = goal.estimate_ex(&MoveContext::route(&insertion_ctx.solution, route_ctx, eval_ctx.job));
    ...
    let (route_costs, best_known_cost) = if let Some(success) = alternative.as_success() {
        match eval_ctx.result_selector.select_cost(route_ctx, success.cost.clone(), route_costs) {
...
fn analyze_insertion_in_route(
    ...
    unwrap_from_result(match insertion_idx {
        ...
        None => eval_ctx.leg_selector.get_legs(route_ctx, eval_ctx.job, init.index).try_fold(init, |acc, leg| {
            analyze_insertion_in_route_leg(eval_ctx, route_ctx, leg, single, target, route_costs.clone(), acc)
        }),
    })
}
...
fn analyze_insertion_in_route_leg(
    ...
    single.places.iter().try_fold(init, |acc, detail| {
        ...
        detail.times.iter().try_fold(acc, |acc, time| {
            ...
            let costs =
                eval_ctx.goal.estimate_ex(&MoveContext::activity(route_ctx, &activity_ctx)) + route_costs.clone();
