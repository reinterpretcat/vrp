...
parameterized_test! {can_solve_solomon_problem_with_cheapest_insertion_heuristic, (problem, expected), {
    can_solve_solomon_problem_with_cheapest_insertion_heuristic_impl(problem, expected);
}}
...
fn can_solve_solomon_problem_with_cheapest_insertion_heuristic_impl(problem: Problem, expected: Vec<Vec<&str>>) {
    let heuristic = create_cheapest_insertion_heuristic();

    let result = heuristic.process(InsertionContext::new(problem));
    let result = result.solution.into_solution(Extras::default());

    assert_eq!(get_customer_ids_from_routes_sorted(&result), expected);
}
