...
pub(crate) fn draw_on_area<B: DrawingBackend + 'static>(
    ...
    let target_span = max_values[target_idx] - min_values[target_idx];
    let padding = if target_span.abs() <= Float::EPSILON {
        (min_values[target_idx].abs() * 0.05).max(1.)
    } else {
        target_span.abs() * 0.05
    };
    let x_min = min_values[target_idx] - padding;
    let x_max = max_values[target_idx] + padding;
    ...
    let mut chart = ChartBuilder::on(area)
        ...
        .margin((1).percent())
        .build_cartesian_2d(0.0..generations.max(1) as Float, x_min..x_max)?;
    ...
    (0..dimensions).try_for_each(|dim_idx| {
        ...
        chart
            .draw_series(LineSeries::new(
                fitness.iter().map(|(generation, fitness)| {
                    ...
                    let fitness = if dim_idx == target_idx {
                        ...
                        if span.abs() <= Float::EPSILON {
                            (new_min + new_max) * 0.5
                        } else {
                            (value - min_value) / span * (new_max - new_min) + new_min
                        }
                    };
            ...
            .label(fitness_config.labels.get(dim_idx).map(String::as_str).unwrap_or("objective"))
