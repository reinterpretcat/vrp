...
fn get_best_fitness(population: &Greedy<VectorFitness, VectorObjective, VectorSolution>) -> f64 {
...
fn can_keep_best_solution() {
    ...
    let mut population = Greedy::new(objective.clone(), 1, None);
...
fn can_format_empty_population() {
    let population = Greedy::new(create_example_objective(), 1, None);
...
fn can_format_filled_population() {
    ...
    let population = Greedy::new(objective, 1, Some(solution));
...
fn can_select_when_empty() {
    ...
    let population = Greedy::new(objective, 1, None);
...
fn can_compare_individuals() {
    ...
    let population = Greedy::new(objective.clone(), 1, None);
