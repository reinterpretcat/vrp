...
fn get_best_fitness(population: &Elitism<VectorObjective, VectorSolution, DefaultRandom>) -> f64 {
    population.ranked().next().unwrap().0.fitness()
}

fn get_all_fitness(population: &Elitism<VectorObjective, VectorSolution, DefaultRandom>) -> Vec<f64> {
...
) -> (Arc<VectorObjective>, Elitism<VectorObjective, VectorSolution, DefaultRandom>) {
